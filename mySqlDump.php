...
class mySqlDump {
    ...
    public function __construct($type="mysql", $settings = null, $database = null, $user, $pass, $host, $filename = null, $pdo_options = array(PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION))
    {
        $this->db = $database;
        $this->user = $user;
        $this->pass = $pass;
        ...
        if( !empty( $filename ) ){
            $this->fileName = $this->db . '_' . _BACKUP_FILE_;
        } else {
            $this->fileName = $this->db . '_backup';
        }
    }
    ...
    public function start($filename = '')
    {
        ...
        echo "Backup Complete at " . date('D, d M Y H:i:s',time());
    }
...
if( isset( $_GET['start'] ) ){
    ...
    //start dump ( $type, $settings, $database, $user, $pass, $host, $filename (optional) )
    $dump = new mySqlDump( 'mysql', null, 'cos_wp_7489327', 'cos_wp_7489327', 'CD$wJF6Y6wEfA3ViHPMoJqNCL', 'db.perspektivedesigns.com' );
    $dump->start();

} else {
    echo '<h1>Perspektive Designs Database Backup Tool</h1>';
    echo '<p>Please enter your credentials to verify that you have permission to run this script.</p><p>If you do not have your information or are otherwise unsure of what you\'re doing please exit this page immediately.</p>';

    //login form
    echo '<form method="post">
        <p><input type="text" name="username" value="" placeholder="Username"></p>
        <p><input type="password" name="password" value="" placeholder="Password"></p>
        <p><input type="text" name="verification" value="" placeholder="Verification Code"></p>
        <p class="submit"><input type="submit" name="commit" value="Login"></p>
      </form>';
}

/*
//add newly created dump to database
    $mysqli = new mysqli(_DB_SERVER_,_DB_USER_,_DB_PASSWD_,'ees_admin_bak');
    $backup = file_get_contents(_BACKUP_FILE_);
    if(!$mysqli->query($backup)){
        echo $mysqli->error;
    }
*/
?>