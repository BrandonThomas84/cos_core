<?php
/*
Plugin Name: Perspektive Design Paypal Shopping Cart
Version: v1.1.3
Plugin URI: http://perspektivedesigns.com/wordpress/plugins/pd-shopping-cart
Author: Brandon Thomas
Author URI: http://perspektivedesigns.com/
Description: This plugin is based on the WordPress Simple Paypal Shopping Cart v3.9.7. There have been numerous modifications made to it to add functionality on both the frontend and the backend
*/
...
include_once('wpsc-meta-boxes.php');
...
function wpspc_cart_actions_handler()
{
    ...
    else if (isset($_POST['delcart']) && $_POST['product'] !== 'Paypal Service Fee (2.9%)')
    {
        $products = $_SESSION['simpleCart'];
        foreach ($products as $key => $item)
        {
...
function wp_cart_options_page () 
{
    ...
    add_options_page(__("PD Paypal Shopping Cart", "WSPSC"), __("PD Shopping Cart", "WSPSC"), 'manage_options', 'wordpress-paypal-shopping-cart', 'wp_cart_options');  
}
...
function wpsc_admin_scripts(){

    wp_enqueue_script( 'wpsc_admin_javascript', plugin_dir_url( __FILE__ ) . 'wpsc_admin.js', array('jquery') );
    wp_enqueue_script( 'wpsc-reg-events-js', plugin_dir_url( __FILE__ ) . 'reg-events.js', array('jquery') );

    wp_enqueue_style( 'wpsc_admin_style', plugin_dir_url( __FILE__ ) . 'wpsc_admin.css' );

}
add_action( 'admin_enqueue_scripts', 'wpsc_admin_scripts' );

function wpsc_frontend_scripts(){

    wp_enqueue_script( 'wpsc_frontend_javascript', plugin_dir_url( __FILE__ ) . 'wpsc_frontend.js', array('jquery') );
    wp_enqueue_style( 'wpsc_frontend_style', plugin_dir_url( __FILE__ ) . 'wpsc_frontend.css' );

}
add_action( 'wp_enqueue_scripts', 'wpsc_frontend_scripts' );
...
function add_wpsc_product_meta_boxes() {

    add_meta_box( 'wpsc-product-meta', 'Product Info', 'wpsc_product_meta_box', 'wpsc_product', 'normal', 'high' );
}
...
//control the addition f paypal service fees
function paypal_service_fee_intercept(){

    //check for cart submission and add fee
    if( isset( $_POST['product'] ) ){

        if( isset ($_SESSION['simpleCart'] ) ){

            //add the individual items to an array
        ...
        if( !empty( $items ) ){
            ...
            $itemTitle = 'Paypal Service Fee (2.9%)';
            //check if fee is in array
            if( in_array( $itemTitle, $items ) ){

                //get the array key for the fee
                $key = array_search( $itemTitle, $items );

                //unset the fee
                unset( $_SESSION['simpleCart'][ $key ] );
            }

            //get the total price without the fee
            $cost = 0;
            foreach( $_SESSION['simpleCart'] as $itemCost  ){
                $cost = $cost + ( $itemCost['price'] * $itemCost['quantity'] );
            }

            //set the price to be 2.9% of the total
            $price = $cost * .029;
            ...
            $fee = array( 
                ...
                'quantity'      => 1,
                'shipping'      => 0,
                'cartLink'      => get_bloginfo('url') . '/registration/convention-registration/',
                'item_number'   => 0 
                );

            //add the fee to the session array
            array_push($_SESSION['simpleCart'], $fee);
        }
    }       
}

//create custom post types for products
function wpsc_product_post_type(){

    //creates the registration events menu item and post type
    register_post_type( 'wpsc_product',
        array(
            'labels' => array(
                'name' => __( 'Cart Products' ),
                'singular_name' => __( 'Cart Product' ),
                'add_new_item' => 'New Cart Product',
                'edit_item' => 'Edit Cart Product'
            ),
            'public' => true,
            'has_archive' => true,
            'description' => 'Once created these products will appaear in the store using the product display shortcode.',
            ...
            'register_meta_box_cb' => 'add_wpsc_product_meta_boxes',
            'supports' => array(
                    'title',
                    'editor',
                    'thumbnail',
...
//create the reporting menu
function _wpsc_reporting_menu(){
    
    add_submenu_page( 'edit.php?post_type=wpsc_cart_orders', 'Order Reports', 'Reporting', 'publish_posts', 'wpsc-order-reporting', '_wpsc_order_reporting' );
}
//control the remporting menu output
function _wpsc_order_reporting(){
    require_once('wpsc.order_reporting.class.php');

    //instantiate reg reporting class
    $reporting = new WPSC_Registration_Reporting;

    //output page display
    $reporting->pageDisplay();
}
//clean a string of characters
...
//adds custom reporting column to the product post type
function product_order_detail_columns($columns) {
    var_dump( $columns );
...
add_action( 'init', 'wpsc_product_post_type' );

//adds paypal service fee if enabled
add_action( 'init', 'paypal_service_fee_intercept');
...
//add_filter('manage_wpsc_product_posts_custom_column','product_order_detail_columns');
...
add_shortcode('wpsc_product_display', 'wpsc_product_display');
...
add_action( 'admin_menu', '_wpsc_reporting_menu');

add_action('add_meta_boxes', 'add_wpsc_product_meta_boxes');
add_action('wp_head', 'wp_cart_css');
add_action('wp_head', 'wp_cart_add_read_form_javascript');
add_action('save_post', 'wpsc_save_product_meta_box'); 
