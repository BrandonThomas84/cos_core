...
function wpspc_order_review_meta_box($wpsc_cart_orders)
{
    ...
    $order_notes = get_post_meta( $wpsc_cart_orders->ID, 'wpsc_order_notes', true );

    //convert order notes
    $converted_notes = '';
    if(!empty($order_notes)){
        foreach( $order_notes AS $date => $note ){
            $converted_notes .= '<li class="wpsc-order-note"><span>' . date( 'Y-m-d H:i:s', $date ) . '</span>' . $note . '</li>';
        }
    }
    ?>
    <table>
        ...
        <tr>
            <td colspan="2">
                <p><?php _e("Add New Order Note:", "WSPSC");?></p>
                <input type="text" size="40" name="wpsc_order_notes" />
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <p><?php _e("Order Notes:", "WSPSC");?></p>
                <ul id="wpsc-order-notes-container"><?php echo $converted_notes; ?></ul>
            </td>
        </tr>
...
function wpspc_cart_save_orders( $order_id, $wpsc_cart_orders ) {
    ...
    if ( $wpsc_cart_orders->post_type == 'wpsc_cart_orders' ) {
        ...
        if ( isset( $_POST['wpsc_order_notes'] ) && $_POST['wpsc_order_notes'] != '' ) {

            //get the existing order notes
            $order_notes = get_post_meta( $wpsc_cart_orders->ID, 'wpsc_order_notes', true );
            
            //if there are no notes then create an empty array
            if(empty($order_notes)){
                $order_notes = array();
            }

            //add new order notes with current date
            $order_notes[ date('U') ] = $_POST['wpsc_order_notes'];

            //update the order notes with the new value
            update_post_meta( $order_id, 'wpsc_order_notes', $order_notes );
        }
    }
}
