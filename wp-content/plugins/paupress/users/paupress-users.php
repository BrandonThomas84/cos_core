 ...
 */
function paupress_insert_user( $args = '' ) {
	...
	*/
	...
	// SCRUB!
	$ivals = paupress_scrub( 'paupress_sanitize', $ivals );
	...
	if ( false != $update && false != $match ) {
		...
		if ( !in_array( $match, $wp_match_reserve ) ) {
			//$wpdb->flush();
			global $wpdb;
			$match_value = $wpdb->get_results( $wpdb->prepare( "SELECT $wpdb->usermeta.user_id FROM $wpdb->usermeta WHERE $wpdb->usermeta.meta_value = %s", $ivals['paupress_user_meta'][$match] ), ARRAY_N );
			$wpdb->flush();
	...
	if ( !empty( $ivals['user_login'] ) ) {
		...
		$username_prefix = get_option( '_paupress_username_prefix' );
		$upre = '';
		...
		$userdata['user_login'] = paupress_random( array( 'name' => $upre, 'compact' => true ) );
	}
	...
	*/
	...
	if ( !empty( $ivals['display_name'] ) ) {
		...
		$dname = '';
		if ( isset( $ivals['paupress_user_meta']['first_name'] ) || isset( $ivals['paupress_user_meta']['last_name'] ) ) {
			if ( isset( $ivals['paupress_user_meta']['first_name'] ) )
				$dname .= $ivals['paupress_user_meta']['first_name'] . ' ';
				
			if ( isset( $ivals['paupress_user_meta']['last_name'] ) )
				$dname .= $ivals['paupress_user_meta']['last_name'];
				
		} else if ( isset( $ivals['paupress_user_meta']['organization'] ) ) {
			$dname .= $ivals['paupress_user_meta']['organization'];
		}
		$userdata['display_name'] = trim( $dname );
	}

	// SET THE NICKNAME
	if ( !empty( $ivals['paupress_user_meta']['nickname'] ) ) {
		$userdata['nickname'] = $ivals['paupress_user_meta']['nickname'];
	} else if ( !$user ) {
		if ( isset( $ivals['paupress_user_meta']['first_name'] ) ) {
			$fname = $ivals['paupress_user_meta']['first_name'];
		} else {
			$fname = '';
		}
	...
	if ( !empty( $ivals['show_admin_bar_front'] ) ) {
		...
		$sabf = paupress_get_option( 'show_admin_bar_front' );
	...
	paupress_update_user_metadata( array( 'user_id' => $user_id, 'uvals' => $ivals, 'data_handler' => $data_handler ) );
 ...
 * @return int/arr The ID if insertion was successful, otherwise a WP_Error.
 */
function paupress_update_user_metadata( $args = '' ) {
	...
	if ( false != $source ) {
		do_action( 'paupress_trigger_user_metadata', $user_id, $uvals, $source );
	}
		
	// SANITIZE THE DATA
	$uvals = paupress_scrub( 'paupress_sanitize', $uvals );
	
	// PROCESS THE USER META / MERGE TAGS
	// IF LOGIC PREVENTS NASTY NOTICES IF THEY ACCIDENTALLY HIT 'EDIT'
	if ( isset( $uvals['paupress_user_meta'] ) ) {			
	
		// GO AHEAD AND PROCESS THE FULL SUBMISSION
		foreach ( $uvals['paupress_user_meta'] as $key => $value ) {
			
			$paupress_{$key} = paupress_get_option( $key );
			if ( empty( $paupress_{$key} ) )
				continue;
			
			// IF THIS IS A SECTION, SKIP IT
			if ( 'section' == $paupress_{$key}['options']['field_type'] )
				continue;
			
			if ( is_array( $value ) ) {
				
				// FOR MULTI-TEXT LOOP THROUGH TO MAKE SURE NULL VALUES ARE REMOVED FROM PROFILE EDITS
				if ( 'multitext' == $paupress_{$key}['options']['field_type'] ) {
					$new_value = array();
				...
				// FOR TAXONOMIES
				if ( 'taxonomy' == $paupress_{$key}['options']['field_type'] ) {

					if ( isset( $uvals['paupress_user_taxonomy_options'] ) )
						$data_handler = $uvals['paupress_user_taxonomy_options'][$key];
					
					if ( 'append' == $data_handler || 'append_no_overwrite' == $data_handler ) {
						$pre_array = get_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ), true );
						if ( is_array( $pre_array ) ) {
							...
							//update_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ), $merged );
						} else {
							//update_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ), $value );
						}
			...
			if ( 'wpr' == $paupress_{$key}['source'] && 'url' == $key ) {
				...
				if ( false !== strpos( $data_handler, 'no_overwrite' ) ) {
					${$key} = get_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ), true );
					if ( '' != ${$key} )
						continue;
				}
				
				if ( empty( $value ) ) {
					delete_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ) );
				} else {
					update_user_meta( $user_id, paupress_key( $key, $paupress_{$key}['source'] ), $value );
				}
				
			}
			/*} elseif ( 'paupress' != $paupress_{$key}['source'] ) {
				...
				// CHECK TO MAKE SURE WE'RE ALLOWING OVERWRITES
				if ( isset( $_POST['no_overwrite'] ) ) {
					${$key} = get_user_meta( $user_id, 'paupress_'.$key, true );
					if ( '' != ${$key} )
						continue;
				}
	
				if ( '' != $value || !empty( $value ) ) {
					update_user_meta( $user_id, 'paupress_'.$key, $value );
				} else {
					delete_user_meta( $user_id, 'paupress_'.$key );
				}
			}*/
	...
	if ( isset( $uvals['paupress_user_taxonomy'] ) ) {
		foreach ( $uvals['paupress_user_taxonomy'] as $key => $value ) {
	...
	*/
	...
	do_action( 'paupress_update_user_metadata_ext', $user_id, $uvals );
 ...
 * @param $paupress_cap str. The paupress-specific capability
 * @param $plu
 * @param $param
 * @param $param
 * @return return type
...
function paupress_hide_meta( $args = '' ) {
	...
	if ( '-bulk-edit' == $action || 'bulk-edit' == $action ) {
		...
		$allow = get_option( '_paupress_bulk_edit_allow' );
		if ( false == $allow )
			$allow = array();
...
/*
function paupress_get_field_status( $val, $key, $args ) {
...
function paupress_get_field_break( $meta_key, $user_meta, $args = array(), $attribute ) {
	if ( empty( $args ) ) {
		return $meta_key;
		...
		foreach ( $args as $key => $val ) {
			if ( 'value' != $attribute ) {
				if ( array_walk_recursive( $user_meta, 'paupress_get_field_status', $val ) )
					return $meta_key;
			} else {
...
function paupress_get_user_metadata( $args = '' ) {
	...
	$paupress_user_meta_arr = array();
	
	foreach ( $paupress_user_meta as $key => $val ) {
	
		// IF IT'S IN A COLUMN
		if ( is_array( $val ) ) {
			foreach ( $val as $subkey => $subval )
				$paupress_user_meta_arr = paupress_get_user_metadata_val( $paupress_user_meta_arr, $subval, $args );
				
		// IF IT'S FREE
		} else {
			$paupress_user_meta_arr = paupress_get_user_metadata_val( $paupress_user_meta_arr, $val, $args );
		}
	...
	if ( !empty( $include ) ) {
		foreach ( $paupress_user_meta_arr as $akey => $aval ) {
			if ( !in_array( $aval['options']['field_type'], $include ) )
				unset( $paupress_user_meta_arr[$akey] );
	...
	return array_filter( $paupress_user_meta_arr );
	
}


// LOCALLY FILTER THE ARRAY
function paupress_get_user_metadata_val( $paupress_user_meta_arr, $val, $args ) {
	
	// GRAB THE META DATA
	$user_meta = paupress_get_option( $val );
	...
	if ( 'section' == $user_meta['options']['field_type'] ) {
		...
		if ( false != $args['section_break'] ) {
			$paupress_user_meta_arr[] = paupress_val( $user_meta, $args['return_val'] );
		} else {
			...
			if ( isset( $user_meta['options']['choices'] ) && !empty( $user_meta['options']['choices'] ) ) {
				foreach ( $user_meta['options']['choices'] as $section_key => $section_val ) {
					...
					// GRAB THE META DATA
					$section_meta = paupress_get_option( $section_val );
					...
					if ( 'group' == $section_meta['options']['field_type'] ) {
						...
						if ( false != $args['group_break'] ) {
							$paupress_user_meta_arr[] = paupress_val( $section_meta, $args['return_val'] );
						} else {
							// LOOP THROUGH ITS CHILDREN
							foreach ( $section_meta['options']['choices'] as $group_key => $group_val ) {
								$paupress_user_meta_arr[] = paupress_val( paupress_get_option( $group_val ), $args['return_val'] );
							}
						}
...
function paupress_primary_marker( $meta, $user_id = '' ) {
	
	if ( isset( $meta['group_type'] ) && 'address' == $meta['group_type'] ) {
	
		$prim_test = get_user_meta( $user_id, 'paupress_pp_primary', true );
		...
		?>
			<span class="paupress-qualifier">
				<input type="radio" name="paupress_user_meta[pp_primary]" class="paupress-primary" value="<?php echo $key; ?>"<?php echo $p_check; ?> /> <span><?php _e( 'primary', 'paupress' ); ?></span>
			</span>
		<?php 
	}
