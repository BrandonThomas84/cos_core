...
/**
 * This is the wrapper function for the PauPress Options API. Your function might be called {prefix}_options.
 *
 * @since 1.0.2
 *
...
function paupress_options() {

	// THE NAVIGATION
	// THE KEYS SHOULD BE NAMED PLACEHOLDERS FOR THE FUNCTION THAT RETURNS AN ARRAY OF FIELD PARAMETERS
	$navigation = apply_filters( 'paupress_options_tabs', array( 
		
		'paupress_general_settings' => array( 
									'title' => __( 'General', 'paupress' ), 
									'subs' => false, 
							),
		
		'paupress_pro_settings' => array( 
									'title' => __( 'Pro', 'paupress' ), 
									'subs' => false, 
							),
							
		'paupress_user_settings' => array( 
									'title' => __( 'Users', 'paupress' ), 
									'subs' => false, 
							),
		
		'paupress_actions_settings' => array( 
									'title' => __( 'Actions', 'paupress' ), 
									'subs' => false, 
							),
							
		'paupress_system_settings' => array( 
									'title' => __( 'System', 'paupress' ), 
									'subs' => false, 
							),
							
		'paupress_forms_settings' => array( 
									'title' => __( 'Forms', 'paupress' ), 
									'subs' => false, 
							),
									
		'paupress_panel_settings_trans' => array( 
									'title' => __( 'Panels', 'paupress' ), 
									'subs' => false, 
							),
	) );
	
	paupress_options_form( __( 'PauPress Options', 'paupress' ), $navigation );

}


/**
 * This function saves PauPress Options data at init to catch taxonomic options.
 *
 * @since 1.0.2
 *
 * @param none
...
function paupress_options_save() {	
	...
	if ( isset( $_POST['_pp_option'] ) ) { 
		...
		// SANITIZE ALL INPUT
		paupress_scrub( 'paupress_sanitize', $_POST );
		
		// LET PLUGINS MANIPULATE THEIR OWN OPTIONS
		do_action( 'paupress_options_save_ext' );
		
		global $notice, $paupress_flush;
		$notice = array();
		$paupress_flush = apply_filters( 'paupress_flush_permalinks', false, $_POST );
...
/**
 * This is the main function for the PauPress Options API.
 *
 * @since 1.0.2
 *
...
function paupress_options_form( $form_title, $tabs = array() ) {	
	...
	if ( !empty( $notice ) )
		echo '<p class="paupress_notice">' . __( 'Settings Updated', 'paupress' ) . ': ' . implode( ', ', $notice ) . '</p>';
...
?>
	...
	<div id="icon-options-general" class="icon32"><br /></div>
		...
		<form enctype="multipart/form-data" action="" method="POST">
			...
			<?php 
				// SET THE NONCE
				wp_nonce_field('paupress-nonce');
				
				// DISPLAY ANY SUB NAVIGATION
				if ( !empty( $tabs[$active]['subs'] ) ) {
					$sub_nav = paupress_options_sub_nav( $active, $tabs[$active]['title'], $tabs[$active]['subs'] );
					echo $sub_nav['nav'];
					$active = $sub_nav['active'];
				}
				
				// FLUSH THE PERMALINKS IF WE NEED TO DO ANY REWRITES
				global $paupress_flush;
				if ( false != $paupress_flush ) {
					flush_rewrite_rules();					
				}
				
				// DO ANY PRE OPTION KINDA THINGS
				do_action( 'paupress_options_pre', $active );
			?>
			...
			<?php
				...
				do_action( 'paupress_options_post', $active );
			?>
			<p class="submit"><input type="submit" name="save_options" value="<?php _e( 'Save Your Options', 'paupress ' ); ?>" class="button-primary" /></p>
...
function paupress_options_sub_nav( $active, $active_title, $subs = array() ) {
	$sub_nav = array();
	$sublinks = array();
	if ( isset( $_GET['sub'] ) ) { 
		$subactive = $_GET['sub'];
...
add_action( 'wp_ajax_paupress_ffc_new_option', 'paupress_ffc_new_option' );
function paupress_ffc_new_option() {
	$context = $_POST['context'];
	$v = $_POST['v'];
	do_action( 'paupress_ffc_option', $context, $v, false );
	die();
}
function paupress_ffc( $args = null ) {
...
?>
	...
	<div id="<?php echo $context; ?>_select" class="options-field options-selector">
		...
		<a id="<?php echo $context; ?>_add_field" class="button add-ffc-field"><?php _e( '+ Add', 'paupress' ); ?></a>
	</div>
						...
						<div class="t-wrapper<?php echo $alt_error; ?>">
							...
							<div id="option-<?php echo $context.$v; ?>" class="inside t-panel" style="display: none;">
								...
								<div id="option-<?php echo $context.$v; ?>-field">
									<?php do_action( 'paupress_ffc_option', $context, $v, $c_array ); ?>
								</div>
							</div>
						</div>
					</li>
		...
		jQuery(document).ready(function() {
			...
			jQuery('#<?php echo $context; ?>_add_field').click(function(){
				...
				jQuery('#option-<?php echo $context; ?>'+fid).each(function(){
					...
					// PLAY THE LOADER
					jQuery('#option-<?php echo $context; ?>'+fid+'-field').empty().html('<div style="padding:10px 0;"><img src="'+paupressAdminAjax.ajaxload+'" /></div>');
						 
					jQuery.post( 
						paupressAdminAjax.ajaxurl, 
						{ action : 'paupress_ffc_new_option', v : fid, context : '<?php echo $context; ?>', paupress_admin_nonce : paupressAdminAjax.paupress_admin_nonce },
					    function( response ) {
