...
function paupress_user_actions() {
	
	return apply_filters( 'paupress_user_actions', array( 
					
			'pp_log' => array( 
										'source' => 'paupress', 
										'type' => 'pp_log', 
										'single' => __( 'Log', 'paupress' ),  
										'plural' => __( 'Logs', 'paupress' ), 
										'class' => 'origin',
										'options' => array( 
															'admin' => true, 
															'user' => false, 
			...
			'pp_interaction' => array( 
										'source' => 'paupress', 
										'type' => 'pp_interaction', 
										'single' => __( 'Interaction', 'paupress' ), 
										'plural' => __( 'Interactions', 'paupress' ), 
										'class' => 'origin',
										'options' => array( 
															'admin' => true, 
															'user' => false, 
...
function paupress_get_init_user_actions() {
	return apply_filters( 'paupress_get_init_user_actions', paupress_user_actions() ); 
}
...
function paupress_get_user_actions() {
	return apply_filters( 'paupress_get_user_actions', paupress_get_init_user_actions() ); 
}
...
function paupress_exclude_user_actions() {
	return apply_filters( 'paupress_exclude_user_actions', array() ); 
}
 ...
 * @return null If you don't see your actions, it didn't work. :)
 */
function paupress_init_user_actions() {
  
  	$paupress_user_actions = paupress_get_init_user_actions();
  	
	foreach ( $paupress_user_actions as $action ) {
		...
		$args['register_meta_box_cb'] = 'paupress_user_actions_meta_box_cb';
...
/**
 * Generic nonce field for all PauPress data.
 *
 * @since 1.0.2
...
function paupress_user_actions_nonce_field() {
	global $post;
	wp_nonce_field( 'paupress-' . $post->post_type.'-meta', 'paupress-' . $post->post_type.'-nonce' );
}


/**
 * Pushes the user actions to PauPress.
 *
 * @since 1.0.2
 * @param arr. The PauPress User Actions Array.
 * @return arr. The filtered Actions Array.
...
function paupress_push_user_actions( $paupress_get_user_actions ) {
	
	$paupress_post_types = get_option( '_paupress_post_types' );
	
	if ( empty( $paupress_post_types ) )
		return $paupress_get_user_actions;
		
	$paupress_get_user_actions = $paupress_post_types;
...
function paupress_get_post_to_edit() {
	...
	if ( isset( $_POST['data'] ) ) {
		...
		if ( false !== strpos( $_POST['data'], 'edit-' ) ) {
			...
			if ( 'user' == $type ) {
				$paupress_actions = paupress_get_user_actions();
			} else {
				$paupress_actions = paupress_get_post_actions();
			}
			$post_type = false;
			$post_val = array();
		...
		if ( 'bulk-edit' == $action ) {
			echo '<ul><li class="meta-item"><span class="paupress-label">';
			echo '<a class="rui off" title="'.$cid.'bulk-edit">Enable Text</a>';
			echo '</span><span class="paupress-field">';
		}
		
		echo '<div style="width: 90%;padding: .3em;margin: .2em 0;">&nbsp;</div>';
		//echo '<p>'. __( 'Title', 'paupress' ) .'<br /><input type="text" name="post_title" class="regular-text" value="'.$post->post_title.'" /></p>';
		//echo '<p>'. __( 'Date', 'paupress' ) .'<br /><input type="text" class="paupress-date" name="post_date" class="regular-text" value="'.$post->post_date.'" /></p>';
		...
		// SET THE META
		if ( 'user' == $type ) {
			paupress_user_actions_meta_fields( array( 'post_id' => $post->ID, 'fields' => paupress_get_user_actions_meta(), 'action' => $action, 'post_val' => $post_val ) );
		} else {
			paupress_post_actions_meta_fields( array( 'post_id' => $post->ID, 'fields' => paupress_get_post_actions_meta(), 'action' => $action, 'post_val' => $post_val ) );
		}
		...
		<?php
		$inline_button = apply_filters( 'paupress_inline_do_action_button', array(
				'<input type="submit" class="paupress-actions-save button-primary '.$type.'" name="save" value="'.__( 'Save', 'paupress' ).'" />', 
...
function paupress_save_new_post() {
	...
	if ( isset( $_POST['data'] ) ) {
		...
		paupress_save_action_meta( array( 'post_data' => $postarr, 'override' => true ) );		
		$act = get_post( $pid );
		$class = apply_filters( 'paupress_save_new_class', array(), $act );
		...
		jQuery(document).ready(function () {
			...
			var newparent = jQuery('.new-post').parent('.paupress-viewer').attr('id');
...
function paupress_exclude_accepted_post_types( $paucontent_accepted_post_types ) {
	
	$paupress_user_actions = paupress_user_actions();
...
function paupress_exclude_restricted_post_types( $paucontent_restricted_post_types ) {
...
function paupress_modal_action(){
	...
	switch ( $_GET['action'] ) {
		...
		case 'test' :
    		...
    		//paupress_actions_editor( array( 'user_id' => $_GET['ID'], 'paupress_cap' => 'admin', 'action' => 'edit', 'embed' => true, 'embed_id' => 'new-action-mini' ) );
...
function paupress_add_nodes( $wp_admin_bar ) {
	global $current_user;
	$args = array(
	      'id' => 'new-action', 
	      'parent' => 'new-content', 
	      'title' => __( 'PauPress Action', 'paupress' ), 
	      'href' => admin_url( '/users.php?page=paupress_modal_action&amp;action=&amp;ID='.$current_user->ID.'&amp;TB_iframe=true&amp;height=300&amp;width=900' ), 
...
function paupress_action_minified() {
	global $pagenow;
	
	if ( is_admin() ) {
		if ( isset( $_GET['page'] ) && 'paupress_modal_action' == $_GET['page'] ) {
	    	//add_action( 'admin_head', 'paupress_action_minify' );
	    }
	}
	
}

function paupress_action_minify() {
	if ( isset( $_GET['ID'] ) ) {
		echo "<style>\r\n";
