...
function profile_fields() { 
	...
	jQuery('#paupress-overlay').prepend('<div id="paupress-modal">'+paupressAdminAjax.oneMoment+'</div>');
    
    // SUBMIT THE FORM DATA
    jQuery.post( 
    	
    	paupressAdminAjax.ajaxurl, { 
    		action : 'paupress_profile_fields_update', 
    		data : str, 
    		paupress_admin_nonce : paupressAdminAjax.paupress_admin_nonce 
    	},
        function( response ) {
            // DISPLAY THE RESPONSE
            jQuery('#paupress-modal').html(response);
            //tb_show(null,'#TB_inline?height=500&width=630&inlineId=paupress-ajax-loading&modal=true',null);
            setTimeout(function() {
            	jQuery('#paupress-modal').fadeOut('slow');
            	//tb_remove();
            	window.location = '';
            },  3000);
        }
...
function new_meta_assign(){
	...
	if ( jQuery('#'+metakey+'-ele').length ) {
		...
		// LET THE USER KNOW
		if ( !jQuery('#message-'+conkey+' .paupress-mk-error').length ) {
			jQuery('#message-'+conkey).append('<span class="paupress-mk-error required example-text">'+paupressAdminAjax.metaKeyTaken+'</span>');
		...
		if ( jQuery('#message-'+conkey+' .paupress-mk-error').length ) {
			jQuery('#message-'+conkey+' .paupress-mk-error').remove();
		...
		jQuery('#t-'+conkey+' [name*="paupress_user_meta_options"]').each(function(i) {
			var narr = jQuery(this).attr('name'); 
			var newarr = narr.replace('['+oldkey+']','['+metakey+']');
			jQuery(this).attr('name',newarr);
		});
		jQuery('#t-'+conkey+' [name*="paupress_user_taxonomy_options"]').each(function(i) {
			var narr = jQuery(this).attr('name'); 
			var newarr = narr.replace('['+oldkey+']','['+metakey+']');
			jQuery(this).attr('name',newarr);
		});
...
function remove_meta(){
	...
	jQuery(this).closest('.t-wrapper').find('[name*="paupress_user_meta_options"]').each(function(i) {
		var narr = jQuery(this).attr('name'); 
		var newarr = narr.replace('['+rvar+']','[_paudel_'+rvar+']');
		jQuery(this).attr('name',newarr);
	});
	jQuery(this).closest('.t-wrapper').find('[name*="paupress_user_taxonomy_options"]').each(function(i) {
		var narr = jQuery(this).attr('name'); 
		var newarr = narr.replace('['+rvar+']','[_paudel_'+rvar+']');
		jQuery(this).attr('name',newarr);
	});
...
function field_selection(){
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+primary).empty().html('<img src="'+paupressElemChoicesAjax.ajaxload+'" />');
	 
	jQuery.post( 
		paupressElemChoicesAjax.ajaxurl, 
		{ action : 'paupress_element_choices_form', meta_key : meta_key, field_type : field_type, paupress_element_choices_nonce : paupressElemChoicesAjax.paupress_element_choices_nonce },
	    function( response ) {
	        // DISPLAY THE RESPONSE
...
function delete_form() {
	...
	var answer = confirm( paupressAdminAjax.confirmDelete );
	if (answer) {
		jQuery.post( 
			paupressAdminAjax.ajaxurl, { 
				action : 'paupress_delete_form', 
				data : cid, 
		    	paupress_admin_nonce : paupressAdminAjax.paupress_admin_nonce 
			},
		    function( response ) {
...
function check_profile(event) {
	...
	if ( jQuery('.halt').length != 0 ) {
		...
		if ( !jQuery('#errors').length ) {
			jQuery('#user-form').prepend('<div class="errors">'+paupressAdminAjax.errMsg+'</div>');
		}
...
function paupress_get_post_to_edit() {
	...
	var cid = jQuery('#'+fid+' .paupress-viewer').attr('id').split('_viewer')[0];

	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+fid+' .paupress-viewer').empty().html('<img src="'+paupressAdminAjax.ajaxload+'" />');
	 
	jQuery.post( 
    	paupressAdminAjax.ajaxurl, { 
    	action : 'paupress_get_post_to_edit', 
    	data : val, 
    	uid : uid, 
    	actung : actung, 
    	cid : cid, 
        ...
        function( response ) {
            // DISPLAY THE RESPONSE
            jQuery('#'+fid+' .paupress-viewer').html(response);
            /*jQuery('#'+fid+' .paupress-viewer').find('script').each(function(i) {
            	eval(jQuery(this).text());
            });*/
            //quicktags({id : cid});
            if ( !paupressAdminAjax.firefox ) {
	            tinymce.init(tinyMCEPreInit.mceInit[cid]);
	            tinyMCE.triggerSave();
	        }
	        paupress_bind_events();
            jQuery('.disabled #'+fid+' .paupress-viewer').find('input[type="text"], select, textarea, input[type="radio"]').attr('disabled', 'disabled');
...
function paupress_get_post_to_modify() {
...
function paupress_save_new_post() {
	...
	// EMPTY OUT THE MAIN HOLDER'S ELEMENTS AND PLAY THE LOADER
	jQuery('#'+sid+' div.paupress-viewer').empty().html('<img src="'+paupressAdminAjax.ajaxload+'" />');
	
	jQuery.post( 
		paupressAdminAjax.ajaxurl, { 
		action : 'paupress_save_new_post', 
		data : str, 
		sid : sid,
		type : type,  
		paupress_admin_nonce : paupressAdminAjax.paupress_admin_nonce 
		...
		function( response ) {
		    // DISPLAY THE RESPONSE
		    jQuery('#'+sid+' div.paupress-viewer').html(response);
		    jQuery('#'+sid+' .actions-launcher').val('');
	        /*jQuery('#'+sid+' div.paupress-viewer').find('script').each(function(i) {
	        	eval(jQuery(this).text());
	        });*/
	    }
	);
...
jQuery(document).ready(function () {
			...
			update: function(event, ui) { 
				...
				if ( jQuery(ui.item).closest('ul').hasClass('section-list') ) {
					...
					if ( !jQuery('#'+sid+'-'+uid).length ) {
						...
						jQuery(ui.item).find('.section-input').val(tsid).after('<input id="'+sid+'-'+uid+'" class="section-output" type="hidden" name="paupress_user_meta_options['+tsid+'][options][choices][]" value="'+tuid+'" />');
					}
				}
	...
	// DO IT...
	jQuery('#paupress').on('click', '.rui', rui_toggle);
	...
	jQuery('#tim').click(function(){
		...
		jQuery.post( 
	    	paupressAdminAjax.ajaxurl, 
	    	{ action : 'paupress_new_elements_form', data : data, paupress_admin_nonce : paupressAdminAjax.paupress_admin_nonce },
	        function( response ) {
	...
	// ACTION EDITOR
	jQuery('#paupress').on('change', '.actions-launcher', paupress_get_post_to_edit);
	jQuery('#paupress').on('click', '.profile-actions-edit', paupress_get_post_to_modify);
	jQuery('#paupress').on('click', '.paupress-actions-save', paupress_save_new_post);
