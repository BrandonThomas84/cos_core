	...
	jQuery('.state-province-field').each(function(){
		...
		if ( jQuery('#'+mylopt).closest('.meta-item').children('.paupress-label').hasClass('semi-required') ) {
			if ( 'US' == selopt || 'AU' == selopt || 'CA' == selopt ) {
				jQuery('#'+mylopt).closest('.meta-item').children('.paupress-label').addClass('required');
				jQuery('#'+mylopt).closest('.meta-item').find('.asterix-required').show();
			} else {
				jQuery('#'+mylopt).closest('.meta-item').children('.paupress-label').removeClass('required');
...
jQuery(document).ready(function () {
	...
	jQuery(document).on('focusin', 'input.paupress-date', function(){
		jQuery(this).datepicker({ dateFormat: 'yy-mm-dd', changeMonth: true, changeYear: true, yearRange: 'c-'+paupressAjax.yearLow+':c+'+paupressAjax.yearHigh });	
	});
	...
	*/
	...
	jQuery('#paupress').on('change', '.state-province-field', paupress_selopt);
	jQuery('#paupress').on('change', '.country-field', paupress_selopt);
