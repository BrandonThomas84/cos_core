...
function paupress_get_field( $args = '' ) {
	...
	$defaults = array( 
					...
					'post_val' => false, 
					'paupress_cap' => '', // NEEDED TO PASS THROUGH FOR GROUPS AND SECTIONS
					'required' => false, 
	...
	if ( false != $post_val )
		$post_val = paupress_scrub( 'paupress_sanitize', $post_val );
	...
	if ( 'option' === $type ) {
		...
		if ( 'textarea' == $meta['options']['field_type'] ) {
			$field['value'] = paupress_scrub( 'paupress_esc_html', get_option( $key ) );
		} else {
			$field['value'] = paupress_scrub( 'paupress_esc_attr', get_option( $key ) );
		}
		if ( false != $post_val )
		...
		if ( 'textarea' == $meta['options']['field_type'] ) {
			$field['value'] = paupress_scrub( 'paupress_esc_html', get_post_meta( $id, $key, true ) );
		} else {
			$field['value'] = paupress_scrub( 'paupress_esc_attr', get_post_meta( $id, $key, true ) );
		}
		if ( false != $post_val )
			$field['value'] = paupress_scrub( 'paupress_esc_attr', $post_val );
		$public = false;
		...
		$field_pre_name = 'paupress_user_meta';
		...
		if ( false != $post_val ) {
			...
			if ( 'wpr' == $meta['source'] ) {  
				...
				$field['value'] = paupress_scrub( 'paupress_esc_attr', get_user_meta( $id, $key, true ) );
			} else {
				if ( 'textarea' == $meta['options']['field_type'] ) {
					$field['value'] = paupress_scrub( 'paupress_esc_html', get_user_meta( $id, 'paupress_'.$key, true ) );
				} else {
					$field['value'] = paupress_scrub( 'paupress_esc_attr', get_user_meta( $id, 'paupress_'.$key, true ) );
				}
			}
		...
		$user_opt = paupress_scrub( 'esc_attr', get_user_meta( $id, 'paupress_pp_public', true ) );
		if ( false != $public ) { 
			...
			// IF A USER HAS NOT DECIDED YET ON THEIR PUBLIC STATUS...
				...
				$admin_opt = get_option( '_paupress_public' );	
				if ( 'true' != $admin_opt ) {
					$pmt = 'off';
					$pmv = 'false';
				} else {
	...
	if ( '-view' == $action || 'view' == $action ) {
		...
		$field['action'] = '-view';
		$label_wrap = false;
		if ( 'true' == get_option( 'paupanels_public_labels' ) ) {
			$label = '<span class="paupanels-profile-label">'.$label.'</span>';
		} else {
			$label = false;
		}			
		$help = false;
	...
	$field['capabilities'] = $paupress_cap;
	
	if ( false != $required || !empty( $meta['options']['req'] ) ) {
		if ( 'paupress' == $meta['source'] && false !== strpos( $meta['meta_key'], 'address_state_' ) ) {
	...
	if ( false != $required && 'checkbox' == $meta['options']['field_type'] && false == $label )
		$mayberequired = __( 'Required', 'paupress' );
	...
	if ( $wrap ) echo '<li class="meta-item">';
		...
		if ( $label_wrap ) echo '<span class="paupress-label'.$required.'">'.$mayberequired;
			...
			if ( $admin_only ) echo '<a class="icon-admin" title="' . __( 'Only visible to admins.', 'paupress' ) . '">&nbsp;</a>';
			if ( $public ) echo '<a class="pmt '. $pmt .'" rel="' . __( 'Public or Private', 'paupress' ) . '" title="public_'. $key .'"><input type="hidden" id="public_'. $key .'"  name="' . $field_pre_name . '[pp_public]['. $key .']" value="'. $pmv .'" />&nbsp;</a>';
		if ( $label_wrap ) echo '</span>';
		
		if ( $label_wrap ) echo '<span class="paupress-field">';
		
			paupress_field( $meta, $field );
		if ( $help && 'checkbox' == $meta['options']['field_type'] ) echo '<a class="help" title="' . $help . '">&nbsp;</a>';
		if ( $label_wrap ) echo '</span>';
...
function paupress_field( $meta, $args = array() ) {
	...
	switch( $meta['options']['field_type'] ) :
		...
		case 'title' : // BUMP
			...
			echo '<h3 class="paupress-section">' . stripslashes( $meta['name'] ) . '</h3>';
		...
		case 'taxonomy' :
			...
			if ( '-view' == $action ) {
				...
				$display = get_option( '_paupress_taxonomy_display' );
				...
				// SPECIAL CASES	
				if ( '-bulk-edit' == $action )
					echo '<div style="clear: both; padding: 7px;"><input type="radio" name="paupress_user_taxonomy_options[' . $key . ']" value="append" checked="checked" /> ' . __( 'Append', 'paupress' ) . '<input type="radio" name="paupress_user_taxonomy_options[' . $key . ']" value="overwrite" /> ' . __( 'Overwrite', 'paupress' ) . '<input type="radio" name="paupress_user_taxonomy_options[' . $key . ']" value="remove" /> ' . __( 'Remove', 'paupress' ) . '</div>';
				
				if ( '-edit' == $action && current_user_can( 'list_users' ) )
					echo '<div class="end-of-float"><a href="' . get_bloginfo('wpurl') . '/wp-admin/edit-tags.php?taxonomy=' . $tax . '&TB_iframe=true&height=450&width=920" class="thickbox button-primary">' . __( 'Add terms', 'paupress' ) . '</a></div>';
		...
		case 'section' :
			...
			if ( '-view' != $action || 'true' == get_option( 'paupanels_public_labels' ) )
			...
			if ( !empty( $meta['options']['choices'] ) ) {
				...
				foreach ( $meta['options']['choices'] as $skey => $sval ) { 
					...
					// GET THE OPTION
					$smeta = paupress_get_option( $sval );
					
					// CHECK TO SEE IF ANY FIELDS HAVE BEEN REMOVED FROM DISPLAY
					$grouped_hide = paupress_hide_meta( array( 'meta' => $smeta, 'paupress_cap' => $capabilities, 'action' => $action ) );
					if ( $grouped_hide )
						continue;
					...
					// ECHO OUT THE FIELDS!
					paupress_get_field( array( 'meta' => $smeta, 'id' => $cid, 'action' => $action, 'type' => $type, 'post_val' => $post_val ) );
		...
		case 'textarea' :
			...
			if ( '-view' == $action ) {
				...
				if ( isset( $meta['options']['wp_editor'] ) && false != $meta['options']['wp_editor'] ) {
					wp_editor( html_entity_decode( $value ), $key, array( 'textarea_name' => $name ) );
				} else {
		...
		case 'multiselect' :
			...
			if ( '-view' == $action ) {
				...
				if ( '-search' == $action ) {
					...
					$multiple = ' multiple="multiple" data-placeholder="' . __( 'All Options', 'paupress' ) . '" style="width: 150px"';
					$name = $name . '[]';
				...
				if ( 'wpr' == $meta['source'] && 'role' == $key ) {
					...
					paupress_dropdown_roles( $user_role );
				...
				// AND DISPLAY NAMES (ADDING A FEW OPTIONS ALONG THE WAY)
					...
					if ( is_user_logged_in() && !empty( $value ) ) {
						...
						// FOR ORGS
						if ( !empty($cuser->paupress_organization) )
							$public_display['display_organization'] = $cuser->paupress_organization;
							
						if ( !empty($cuser->first_name) && !empty($cuser->last_name) ) {
							
							// SETUP FORMAL VARS
							if ( !empty( $cuser->paupress_prefix ) )
								$m_pre = $cuser->paupress_prefix . ' ';
								
							if ( !empty( $cuser->paupress_suffix ) )
								$m_suf = ' ' . $cuser->paupress_suffix;
								
							if ( !empty( $cuser->paupress_middle_name ) ) {
								$m_mid = ' ' . $cuser->paupress_middle_name . ' ';
							} else {
								$m_mid = ' ';
							}
		...
		case 'checkbox' :
			...
			if ( '-bulk-edit' == $action ) {
				...
				echo '<option value="true">' . __( 'yes', 'paupress' ) . '</option>';
				echo '<option value="false">' . __( 'no', 'paupress' ) . '</option>';
				echo '</select>';
					
			} else if ( '-search' === $action ) {
			
				echo '<select name="' . $name . '">';
				echo '<option value=""' . selected( $value, '' ) . '>' . __( 'empty', 'paupress' ) . '</option>';
				echo '<option value="true"' . selected( $value, 'true' ) . '>' . __( 'yes', 'paupress' ) . '</option>';
				echo '<option value="false"' . selected( $value, 'false' ) . '>' . __( 'no', 'paupress' ) . '</option>';
				echo '</select>';
		...
		case 'group' :
			...
			if ( '-view' == $action ) {
				...
				foreach ( $meta['options']['choices'] as $gkey => $gvalue ) { 
					...
					$grouped_hide = paupress_hide_meta( array( 'meta' => $gmeta, 'paupress_cap' => $capabilities, 'action' => $action ) );
					if ( $grouped_hide )
						continue;
					...
					paupress_get_field( array( 'meta' => $gmeta, 'id' => $cid, 'action' => $action, 'type' => $type, 'post_val' => $post_val ) );
				}
				
			} else { 
			
				$group_hide = paupress_hide_meta( array( 'meta' => $meta, 'group_override' => true, 'paupress_cap' => $capabilities, 'action' => $action ) );
				if ( false == $group_hide ) {
					?>
					<li>
						<div class="t-wrapper group">
							<p class="t-trigger-wrapper group">
								<span class="paupress-label group">
								...
								<?php
									...
									do_action( 'paupress_group_markers', $meta, $cid, $action );
								?>
								</span>
							</p>
							<div id="<?php echo $id.$action; ?>" class="t-panel">
								...
								<?php 
									foreach ( $meta['options']['choices'] as $gkey => $gvalue ) { 
										...
										// GET THE OPTION
										$gmeta = paupress_get_option( $gvalue );
				...
				*/
					...
					foreach( $value as $fkey => $fval ) {
						...
						if ( false != $add_fields ) {
							...
							foreach ( $meta['options']['add_fields'] as $k => $v ) {
								...
								if ( 'text' == $v['type'] ) {
									if ( isset( $v['class'] ) ) $class = $class . ' ' . implode( ' ', $v['class'] );
									$add_fields .= '<input type="text" name="' . $name . '['.$nfkey.']['.$k.']" class="regular-text regular-multi' . $class . '" style="width: '.$fper.'%;" value="' . $ctv . '" placeholder="'.$add_name.'" /> ';
					...
					?>
						<li id="<?php echo $id .''. $fkey; ?>" class="multilist">
							...
							<span>
								<?php echo $add_fields; ?>
								<input type="text" name="<?php echo $name . '['.$nfkey.'][value]'; ?>" class="regular-text regular-multi<?php echo $class; ?>" style="width: <?php echo $fper; ?>%;" value="<?php echo $value[$fkey]['value']; ?>" placeholder="<?php _e( 'Value', 'paupress' ); ?>" />
								<?php echo $key_type; ?>
							</span> 
							<?php if ( $arrcount == 1 ) { ?>
		...
		case 'password' :
			...
			if ( '-view' != $action ) {
				...
				if ( 'wpr' == $meta['source'] ) {
					?>
					<li><input type="password" autocomplete="off" name="<?php echo $key.$choices[0]; ?>" id="<?php echo $key.$choices[0].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 'type password', 'paupress' ); ?></li>
					<li><input type="password" autocomplete="off" name="<?php echo $key.$choices[1]; ?>" id="<?php echo $key.$choices[1].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 're-type password', 'paupress' ); ?></li>
					<?php
				} else {
					?>
					<li><input type="password" autocomplete="off" name="<?php echo $name; ?>[<?php echo $choices[0]; ?>]" id="<?php echo $key.$choices[0].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 'type password', 'paupress' ); ?></li>
					<li><input type="password" autocomplete="off" name="<?php echo $name; ?>[<?php echo $choices[1]; ?>]" id="<?php echo $key.$choices[1].$action; ?>" class="regular-text<?php echo $class; ?>" value="" /> <?php _e( 're-type password', 'paupress' ); ?></li>
					<?php
				}
		...
		case 'date' :
			...
			echo '<input type="text" name="'.$name.'" id="'.$id.'" class="regular-text paupress-date'.$class.'" value="'.$value.'" />';
		...
		case 'text' :
			...
			if ( '-view' == $action ) { 
				...
				if ( '-search' == $action && 'user_registered' == $key ) {
					...
					echo '<input type="text" name="'.$name.'[]" id="'.$id.'_start" placeholder="'.__( 'start', 'paupress' ).'" class="regular-text paupress-date'.$class.'" value="'.$vs.'" />';
					echo '<input type="text" name="'.$name.'[]" id="'.$id.'_end" placeholder="'.__( 'end', 'paupress' ).'" class="regular-text paupress-date'.$class.'" value="'.$ve.'" />';
					...
					echo $value . '<br />';
					if ( 'ID' == $key )
						echo implode( '<br />', apply_filters( 'paupress_user_id_assets', array(), $value, $action ) );
		...
		default :
			do_action( 'paupress_field_switch', $meta, $args );
		break;
