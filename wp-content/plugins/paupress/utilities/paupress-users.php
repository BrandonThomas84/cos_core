...
function paupress_init_roles() {
	global $wp_roles;
	$paupress_user_actions = paupress_get_init_user_actions();
	$paupress_post_actions = paupress_get_init_post_actions();
	$paupress_actions = array_merge( $paupress_user_actions, $paupress_post_actions );
	if ( isset( $wp_roles ) ) {
...
function paupress_role_capabilities( $status, $role ) {
...
function paupress_user_can( $action, $args = null ) {
	...
	switch( $action ) {
		...
		case 'edit_user' :
			...
			if ( user_can( $one, 'list_users' ) ) {
				...
				return apply_filters( 'paupress_user_can_edit_user', true, $one, $two );	
			}
		...
		default :
			do_action( 'paupress_user_can_switch', $action, $one, $two );
		break;
...
function paupress_email_authenticate( $user, $username, $password ) {
...
function paupress_address_corrections( $args = '' ) {
	...
	if ( false !== strpos( $key, 'address_country' ) ) {
		$paupress_helper_country = paupress_helper_country();
		if ( strlen( $value ) > 3 ) {
			$value = in_array_r( $value, $paupress_helper_country, true );
		} else {
			$value = substr( $value, 0, 2 );
		}
		 
	} else if ( false !== strpos( $key, 'address_state' ) ) {
		$paupress_helper_state = paupress_helper_state();
		if ( strlen( $value ) > 3 ) {
			$top_value = in_array_r( $value, $paupress_helper_state, true );
			$value = in_array_r( $value, $paupress_helper_state[$top_value], true );
		} else {
			$paupress_helper_country = paupress_helper_country();
...
function paupress_state_lookdown( $key, $value ) {
...
function paupress_admin_body_class( $admin_body_class ) {
	if ( false != get_option( '_paupress_admin_body_class' ) ) {
		$class = explode( ' ', $admin_body_class );
		foreach( $class as $k => $v ) {
			if ( false !== strpos( $v, 'admin-color-' ) ) {
				$class[$k] = 'admin-color-' . get_option( '_paupress_admin_body_class' );
			}
		}
