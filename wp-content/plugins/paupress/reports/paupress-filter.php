...
function paupress_filter_process( $post_data ) {
		...
		if ( !isset( $post_data['all_search'] ) ) {
			...
			if ( isset( $post_data['search'] ) ) {
				...
				foreach ( $post_data['search'] as $key => $value ) {
					...
					if ( 'paupress' != substr( $fkey, 0, 8 ) ) {
						$option_key = 'paupress_' . $fkey;
					} else {
						$option_key = $fkey;	
					}
					...
					if ( 'skip' != $op ) { 
						...
						if ( isset( $value['post_ops'] ) ) {
							...
							foreach ( $value['post_ops'] as $post_op ) {
								...
								$mtjoin[$mtc] = "INNER JOIN $wpdb->usermeta " . $mtc_as . "ON ($wpdb->users.ID = " . $mtc_dot . "user_id)";
								// PREPARE $mtquery[$mtc] = "(" . $mtc_dot . "meta_key = 'paupress_" . $post_op . "')";
								$mtquery[$mtc] = $wpdb->prepare( "(" . $mtc_dot . "meta_key = %s)", "paupress_$post_op" );
						...
						$skip_arr[] = 'paupress_'.$value['field'];
					}
					...
					if ( 'taxonomy' == $user_meta['options']['field_type'] ) {
						...
						if ( 'paupress' != $user_meta['source'] ) {
							$switch_key = $fkey;
						} elseif ( 'paupress' == $user_meta['source'] ) {
				...
				if ( isset( $ometa ) ) {
					...
					if ( in_array( $order_by, $skip_arr ) ) {
						...
						$t_order_by = paupress_get_option( $order_by );
						if ( false != $t_order_by ) {
							if ( 'paupress' == $t_order_by['source'] )
								$order_by = 'paupress_'.$order_by;
						}
						...
						if ( isset( $t_order_by['group'] ) && false !== strpos( $t_order_by['group'], 'address' ) ) {
							...
							$nullquo = $wpdb->prepare( $mtc_dot . "meta_key LIKE %s", '%paupress_'.$addapp.'%' );
						} else {
			...
			do_action( 'paupress_search_extend', $post_data, $all_search );
...
function paupress_query_builder( $key, $value ) {
	
	if ( is_array( $value ) ) {
		foreach ( $value as $subkey => $subvalue ) {
			if ( 'wp' == $paupress_{$key}['source'] ) {
				...
				if ( 'address' == substr( $paupress_{$key}['group'], 0, 7 ) ) {
		...
		if ( 'wp' == $paupress_{$key}['source'] ) {
			...
			if ( isset( $paupress_{$key}['group'] ) && 'address' == substr( $paupress_{$key}['group'], 0, 7 ) ) {
