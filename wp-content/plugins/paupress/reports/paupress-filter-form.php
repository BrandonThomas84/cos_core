...
function paupress_filter_profile( $args = '' ) {
	$defaults = array( 
					'display' => 'open', 
					'search' => array( array() ),
					'order_by' => 'ID', 
...
<div id="filter" class="drawer"<?php if ( 'open' === $display ) { echo ' style="display: block;"'; } ?>>
	<div class="inside">
		<?php 
			$r_subnav = array();
			if ( false != get_option( '_paupress_' . $current_user->ID . '_current' ) )
				$r_subnav[] = '<a href="' . admin_url( 'users.php?page=paupress_reports' ) . '">' . __( 'Re-load last search', 'paupress' ) . '</a>';
			
			$r_subnav[] = '<a href="' . admin_url( 'users.php?page=paupress_reports&reset=true' ) . '">' . __( 'Reset current search', 'paupress' ) . '</a>';
	...
	<form id="filter-form" class="report-form" enctype="multipart/form-data" action="<?php echo admin_url( 'users.php' ); ?>" method="POST">
		...
		<div id="filter-profile"><div class="inside">
			<?php $paupress_user_queries = get_option( '_paupress_user_queries' ); ?>
		...
		<div>
			<div style="padding: 15px;">
				<span class="filter-tab"><?php _e( 'Order', 'paupress' ); ?>: <select name="order"><option value="DESC"<?php selected( $order, 'DESC' ); ?>><?php _e( 'Descending', 'paupress' ); ?></option><option value="ASC"<?php selected( $mod_results, 'ASC' ); ?>><?php _e( 'Ascending', 'paupress' ); ?></option></select></span><span class="filter-tab"><?php 
											_e( 'Order by', 'paupress' );
											echo ': ';
											$ob_ppum = array( 'paupress_ID', 'paupress_user_registered', 'paupress_first_name', 'paupress_last_name', 'paupress_user_login', 'paupress_role' );
											paupress_user_data_select( array( 'meta_key' => $order_by, 'value' => $order_by, 'name' => 'order_by', 'ppum' => $ob_ppum, 'chosen' => false, 's_context' => 'orderby' ) ); 
									?></span>

				<span><?php _e( 'Results Per Page', 'paupress' ); ?>: <input class="input-short small-text" type="text" name="users_per_page" value="<?php echo $users_per_page; ?>" /></span>
				...
				<span style="padding: 15px 15px 15px 0; margin-left: 5px; background-color: #DDD;">
					<span class="filter-tab"><?php _e( 'Match', 'paupress' ); ?>: <select name="mod_results"><option value="AND"<?php selected( $mod_results, 'AND' ); ?>><?php _e( 'All', 'paupress' ); ?></option><option value="OR"<?php selected( $mod_results, 'OR' ); ?>><?php _e( 'Any', 'paupress' ); ?></option></select></span>
		...
		<?php
			$puq_title = '';
			if ( isset( $_GET['query'] ) ) {
				$paupress_user_queries = get_option( '_paupress_user_queries' );
				foreach( $paupress_user_queries as $qk => $qv ) {
		...
		<input type="hidden" name="action[paupress_filter_process]" value="" />
	</form>
</div>
<?php
}
...
function paupress_add_search_row() {
	
	if ( isset( $_POST['data'] ) )
		paupress_search_row( array( 'skey' => $_POST['data'] ) );	
...
function paupress_search_row( $args = '' ) {
	...
	<li id="<?php echo $skey; ?>" class="query-block query_multilist tn-wrapper">
		...
		<span class="query-selections">
			...
			<span id="<?php echo $skey; ?>-parent" class="query-parent-holder" title="<?php echo $skey; ?>">
				<?php paupress_user_data_select( array( 
														'class' => 'query-parent', 
														'meta_key' => $meta_key, 
														'value' => $sval 
												) ); ?>
			...
			<span id="<?php echo $skey; ?>-child" class="query-child-holder">
				<?php paupress_search_form( array( 
													'fid' => $meta_key, 
													'key' => $skey, 
													'query' => $sval 
											) );
...
function paupress_user_data_select( $args = '' ) {
	$defaults = array( 
					'id' => '', 
					'class' => '', 
					...
					's_context' => 'search',
					'ppum' => paupress_get_user_metadata( array( 'group_break' => true ) ), 
					'wpr' => false, 
					'meta_only' => false, 
					'chosen' => ' chzn-select', 
					'multiple' => false, 
	...
	<select class="<?php echo $class . $chosen; ?>"<?php if ( false !== $name ) { echo ' name="' . $name . '"'; } if ( false !== $multiple ) { echo ' multiple="multiple"'; } ?>>
		<option value=""><?php _e( 'Make a selection', 'paupress' ); ?></option>
		<optgroup label="<?php _e( 'Profile Fields', 'paupress' ); ?>">
		<?php
				...
				if ( 'group' === $user_meta['options']['field_type'] || 'section' === $user_meta['options']['field_type'] ) {
					...
					if ( 'address' === $user_meta['group_type'] && false == $address_lock ) {
						...
						if ( false == $address_unlock ) {
							...
							if ( '1' != strrchr( $user_meta['meta_key'], '_' ) ) {
								...
								$fid = $add_app . '1';
								$user_meta = get_option( 'paupress_'.$fid );
							}
					...
					foreach ( $user_meta['options']['choices'] as $group_key => $group_value ) {
						...
						if ( 'section' === $user_meta['options']['field_type'] ) {
							$prefix = '';
							$group_meta = get_option( 'paupress_'.$group_value );
						} else {
			...
			$fput = apply_filters( "paupress_user_data_select_f_{$s_context}", $fput );
			echo implode( '', $fput );
		?>
		</optgroup>
		<?php
			...
			if ( false != $s_context )
				do_action( "paupress_user_data_select_{$s_context}", $meta_key, $wpr );
		?>
	</select>
 <?php
}
...
function paupress_search_form( $args = '' ) {
	
	// SET THE DEFAULTS TO BE OVERRIDDEN AS DESIRED
	if ( isset( $_POST['fid'] ) ) {
		$defaults = array( 
						'nonce' => $_POST['paupress_search_nonce'], 
						'fid' => $_POST['fid'], 
						'key' => $_POST['key'], 
						'query' => array(),
						'ajax' => true 
	...
	$_is = __( 'is', 'paupress' );
	$_not = __( 'not', 'paupress' );
	$_like = __( 'like', 'paupress' );
	$_unlike = __( 'not like', 'paupress' );
	$_null = __( 'empty', 'paupress' );
	$_notnull = __( 'not empty', 'paupress' );
	$_any = __( 'any', 'paupress' );
	$_all = __( 'all', 'paupress' );
	$_show = __( 'show', 'paupress' );
					
	// DIFFERENTIATE BETWEEN META AND ACTION
	$user_meta = paupress_get_option( $fid );
	$user_actions = paupress_get_user_actions();
		...
		<span class="panel">
			<span class="op">
				<select name="search[<?php echo $key; ?>][operator]">
					<option value="min"<?php if ( isset( $query['operator'] ) && in_array( $query['operator'], array( 'is', 'min' ) ) ) { echo ' selected="selected"'; } ?>><?php _e( '>=', 'paupress' ); ?></option>
					<option value="max"<?php if ( isset( $query['operator'] ) && in_array( $query['operator'], array( 'not', 'max' ) ) ) { echo ' selected="selected"'; } ?>><?php _e( '<=', 'paupress' ); ?></option>
				</select>
			...
			<span class="op">
				...
				<input type="text" placeholder="<?php _e( 'Title or Body', 'paupress' ); ?>" name="search[<?php echo $key; ?>][title]" value="<?php if ( isset( $query['title'] ) ) { echo $query['title']; } ?>" style="width: 200px;" />
			</span>
		
			<select class="date-toggle" id="<?php echo $key; ?>_dtoggle" name="search[<?php echo $key; ?>][date][toggle]">
				<option value=""><?php _e( 'Date Options', 'paupress' ); ?></option>
				<option value="<?php echo $key; ?>_ran"<?php if ( isset( $query['date']['toggle'] ) ) selected( $query['date']['toggle'], $key .'_ran' ); ?>><?php _e( 'Date Range', 'paupress' ); ?></option>
				<option value="<?php echo $key; ?>_rel"<?php if ( isset( $query['date']['toggle'] ) ) selected( $query['date']['toggle'], $key .'_rel' ); ?>><?php _e( 'Relative Date', 'paupress' ); ?></option>
			</select>
			<span id="<?php echo $key; ?>_ran" class="date_field"<?php if ( isset( $query['date']['toggle'] ) && $key.'_ran' == $query['date']['toggle'] ) echo ' style="display: inline-block"'; ?>>
				<input id="<?php echo $key; ?>_start" class="paupress-date" type="text" placeholder="Start" name="search[<?php echo $key; ?>][date][range][start]" value="<?php if ( isset( $query['date']['range']['start'] ) ) { echo $query['date']['range']['start']; } ?>" style="width: 80px;" /> <input id="<?php echo $key; ?>_end" class="paupress-date" type="text"  placeholder="End" name="search[<?php echo $key; ?>][date][range][end]" value="<?php if ( isset( $query['date']['range']['end'] ) ) { echo $query['date']['range']['end']; } ?>" style="width: 80px;" />
			</span>
			<span id="<?php echo $key; ?>_rel" class="date_field"<?php if ( isset( $query['date']['toggle'] ) && $key.'_rel' == $query['date']['toggle'] ) echo ' style="display: inline-block"'; ?>>
				<select name="search[<?php echo $key; ?>][date][rel][operator]">
					<option value="min"<?php if ( isset( $query['date']['rel']['operator'] ) && in_array( $query['date']['rel']['operator'], array( 'min' ) ) ) { echo ' selected="selected"'; } else if ( !isset( $query['date']['rel']['operator'] ) || empty( $query['date']['rel']['operator'] ) ) { echo ' selected="selected"'; } ?>><?php _e( '<=', 'paupress' ); ?></option>
					<option value="is"<?php if ( isset( $query['date']['rel']['operator'] ) && in_array( $query['date']['rel']['operator'], array( 'is' ) ) ) { echo ' selected="selected"'; } ?>><?php _e( '=', 'paupress' ); ?></option>
					<option value="max"<?php if ( isset( $query['date']['rel']['operator'] ) && in_array( $query['date']['rel']['operator'], array( 'max' ) ) ) { echo ' selected="selected"'; } ?>><?php _e( '>=', 'paupress' ); ?></option>
				</select>
				...
				<select name="search[<?php echo $key; ?>][date][rel][context]">
					<option value=""><?php _e( 'Time', 'paupress' ); ?></option>
					<option value="days"<?php if ( isset( $query['date']['rel']['context'] ) ) selected( 'days', $query['date']['rel']['context'] ); ?>><?php _e( 'days', 'paupress' ); ?></option>
					<option value="weeks"<?php if ( isset( $query['date']['rel']['context'] ) ) selected( 'weeks', $query['date']['rel']['context'] ); ?>><?php _e( 'weeks', 'paupress' ); ?></option>
					<option value="months"<?php if ( isset( $query['date']['rel']['context'] ) ) selected( 'months', $query['date']['rel']['context'] ); ?>><?php _e( 'months', 'paupress' ); ?></option>
					<option value="years"<?php if ( isset( $query['date']['rel']['context'] ) ) selected( 'years', $query['date']['rel']['context'] ); ?>><?php _e( 'years', 'paupress' ); ?></option>
				</select> 
				<select name="search[<?php echo $key; ?>][date][rel][frame]">
					<option value=""><?php _e( 'Frame', 'paupress' ); ?></option>
					<option value="before"<?php if ( isset( $query['date']['rel']['frame'] ) ) selected( 'before', $query['date']['rel']['frame'] ); ?>><?php _e( 'before', 'paupress' ); ?></option>
					<option value="after"<?php if ( isset( $query['date']['rel']['frame'] ) ) selected( 'after', $query['date']['rel']['frame'] ); ?>><?php _e( 'after', 'paupress' ); ?></option>
				</select> 
				<select class="timeframe-select" title="<?php echo $key; ?>_timeframe-rel-abs" name="search[<?php echo $key; ?>][date][rel][pin]">
					<option value=""><?php _e( 'When', 'paupress' ); ?></option>
					<option value="now"<?php if ( isset( $query['date']['rel']['pin'] ) ) selected( 'now', $query['date']['rel']['pin'] ); ?>><?php _e( 'now', 'paupress' ); ?></option>
					<option value="date"<?php if ( isset( $query['date']['rel']['pin'] ) ) selected( 'date', $query['date']['rel']['pin'] ); ?>><?php _e( 'date', 'paupress' ); ?></option>
				</select>
				<input id="<?php echo $key; ?>_timeframe-rel-abs" class="paupress-date s_date_field" type="text" name="search[<?php echo $key; ?>][date][rel][pindate]" value="<?php if ( isset( $query['date']['rel']['pindate'] ) ) { echo $query['date']['rel']['pindate']; } ?>" style="width: 70px;" />
			</span>
		</span>
		<span class="sub-panel"><?php do_action( 'paupress_action_search_meta', $fid, $key, $query ); ?></span>
		<?php
	...
	// META
		...
		*/	
			...
			<?php 
				$post_ops_arr = apply_filters( 'paupress_post_ops_arr', array( 'pp_primary' => __( 'Primary', 'paupress' ) ) );
			...
			<span class="panel">
				<?php 
					...
					paupress_search_field_manipulation( $args ); 
				?>
			</span>
...
function paupress_search_field_manipulation( $args = null ) {
